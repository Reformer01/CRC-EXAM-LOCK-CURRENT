# CKA Sheet Collaborator Quick Guide

This guide is for staff/invigilators who were added to the CKA Google Sheet so they can (1) grant Apps Script permissions and (2) clear violations / unlock students.

## 1) Get access to the Sheet
- Open the shared CKA Google Sheet link.
...
3. In the top menu bar, click:
   - `CKA Admin` → `Test Email (Authorize MailApp)`
...
2. Click `CKA Admin` → `Clear Violations by Session ID`.
3. Paste the Session ID → `OK`.
4. Confirm the result.

### Clear ALL violations
Use this only if you intend to reset the whole sheet.
1. Click `CKA Admin` → `Clear All Violations`.
...
2. Click `CKA Admin` → `Unlock Session ID`.
3. Paste the Session ID → `OK`.
...
- Permission error sending email:
  - Run `CKA Admin` → `Test Email (Authorize MailApp)` and approve.
...
- Cell limit error when clearing/unlocking:
  - Create a new sheet for the term (see section 7).
  - Update Apps Script to point to the new Sheet ID.
  - Share the new Sheet with collaborators.
- Unlocking a session fails:
  - Ensure Session ID exactly matches `Sessions` tab (copy/paste to avoid typos).
  - Confirm the sheet is the one linked to the Apps Script (check Spreadsheet ID in Project Properties if you recently created a new sheet).
  - Ensure you have at least Editor access; Viewer access cannot run admin menu actions.
  - If the Apps Script was recently redeployed to a new sheet, collaborators must use the new Sheet URL.
  - Try refreshing the Sheet and the Apps Script web app after a few minutes; cache can cause temporary mismatches.

## What you should NOT do for students.
- Do not delete rows manually in `Violations` unless instructed; use the `CKA Admin` menu so resets stay consistent for students.
